<?php
class ControllerExtensionModuleUPTHEMEOPTIONS extends Controller {
	private $error = array();
	...
	public function index() {
		...
		$language = $this->load->language('extension/module/up_theme_options');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			...
			$this->response->redirect($this->url->link('extension/module/up_theme_options&submit=true', 'token=' . $this->session->data['token'], 'SSL'));
		}	
		...
		$data['breadcrumbs'][] = array(
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/module/up_theme_options', 'token=' . $this->session->data['token'], true),
      		'separator' => ' :: '
   		);
		
		$data['action'] = $this->url->link('extension/module/up_theme_options', 'token=' . $this->session->data['token'], true);
		...
		$this->response->setOutput($this->load->view('extension/module/up_theme_options.tpl', $data));
	}
		
	private function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/up_theme_options')) {
