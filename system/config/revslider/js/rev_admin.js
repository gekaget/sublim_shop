...
//end save rev slider module position value
		...
		var updateCatByPostTypes = function(typeSettingName,catSettingName){
                ...
                var commonEvt = function(elem){
					var arrTypes = elem.val();
					...
					jQuery(arrTypes).each(function(index,postType){
						...
						for(catIndex in objCats){
							...
							// if(postType == 'smartblog'){
							// 	var rev_id_product = 'smartblog_'+catTitle['id_smart_blog_category'];
							// 	var rev_name = catTitle['meta_title'];
							// 	var opt = new Option(rev_name, rev_id_product);	
							// }else{
							...
							//select first option:
								if(typeof g_jsonArrSliderParams != 'undefined' && g_jsonArrSliderParams.length > 0){
									jQuery.each(g_jsonArrSliderParams,function(k,v){
										if(v == jQuery(opt).attr('value'))
            ...
            jQuery(window).load(function(){
                commonEvt(jQuery("#"+typeSettingName));
            });
                        
			jQuery("#"+typeSettingName).change(function(){
                            commonEvt(jQuery(this));
			});
		};
		...
		var initSliderViewCommon = function(){
			...
			if (typeof g_jsonTaxWithCats != "string" || !g_jsonTaxWithCats){
				g_postTypesWithCats = null;
        	}else{
		...
		**/
		...
		this.initEditSliderView = function(){
			...
			jQuery("#button_export_slider").click(function(){
				...
				var urlAjaxExport = rev_site_admin_url+"?route=extension/module/revslideropencart/ajaxexecute&action="+g_uniteDirPlagin+"_ajax_action&client_action=export_slider&dummy="+useDummy;
				urlAjaxExport += "&sliderid=" + sliderID+"&token="+sds_rev_oc_token;
				// alert(urlAjaxExport);
		...
		var initSlideListGlobals = function(sliderID){
			...
			jQuery(".col-image .slide_image").click(function(){
				...
				UniteAdminRev.openAddImageDialog(g_messageChangeImage,function(urlImage){					
